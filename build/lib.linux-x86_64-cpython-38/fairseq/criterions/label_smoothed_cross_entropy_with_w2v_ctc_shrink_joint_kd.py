...
@register_criterion("label_smoothed_cross_entropy_with_w2v_ctc_shrink_joint_kd")
class LabelSmoothedCrossEntropywithW2vCtcShrinkJointKD(
    ...
    def forward(self, model, sample, per_task, reduce=True):
        ...
        if per_task == "st":
            ...
            transcript_lengths = transcript["lengths"]
            #print(transcript_tokens)
            kd_mt_out = model.textual_encoder(transcript_tokens, transcript_lengths)
            net_kd_output = model.decoder(
                prev_output_tokens=prev_output_tokens, encoder_out=kd_mt_out
            )
            kd_probs = model.get_normalized_probs(net_kd_output, log_probs=False)
            log_probs = model.get_normalized_probs(net_output, log_probs=True)
            pad_mask = sample["target"].eq(self.padding_idx)
            #print(log_probs.shape, kd_probs.shape)
            kd_loss = F.kl_div(log_probs[~pad_mask], kd_probs[~pad_mask], reduction = 'sum')
            loss = loss + kd_loss
