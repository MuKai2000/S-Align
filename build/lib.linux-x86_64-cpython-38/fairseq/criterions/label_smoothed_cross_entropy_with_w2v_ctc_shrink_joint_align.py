...
@register_criterion("label_smoothed_cross_entropy_with_w2v_ctc_shrink_joint_align")
class LabelSmoothedCrossEntropywithW2vCtcShrinkJointAlign(
    ...
    def __init__(self, task, sentence_avg, label_smoothing, post_process="letter", ctc_weight=0.0, 
                 ...
                 train_st_without_ctc=False, max_position_ctc=0, use_token_contrastive=False, use_two_contrastive=False, add_proj_norm=False):
    ...
    def forward(self, model, sample, per_task, reduce=True):
        ...
        if per_task == "st":
            ...
            src_tokens, src_lengths, prev_output_tokens = sample["net_input"].values()
            encoder_out = model.acoustic_encoder(src_tokens, src_lengths)
            loss, tmp_logging_output = self.compute_ctc_loss(model, sample, encoder_out,True)
            sample_size = (
                sample["target"].size(0) if self.sentence_avg else sample["ntokens"]
            )
            logging_output = {
    ...
    def compute_ctc_loss(self, model, sample, acoustic_encoder_out, wav_ctc=False, only_contrastive=False):
        ...
        if self.max_position_ctc != 0:
            ...
            pos_probs = torch.sigmoid(acoustic_encoder_out["pos_wav2vec_out"][0].float())
            pos_lprobs = torch.log(torch.cat((1 - pos_probs, pos_probs), -1))
        ...
        if only_contrastive:
            ...
            if self.max_position_ctc != 0:
                gama = min(math.ceil(model.acoustic_encoder.num_updates / 3000) * 0.1, 1.0)
                #gama=0.7
                #if gama <= 0.9: 
                ...
                target_pos = (lprobs.argmax(dim=-1) != 0).float()
                #print(target_pos.shape, acoustic_encoder_out["pos_wav2vec_out"][0].shape)
                align_loss = F.binary_cross_entropy_with_logits(
                    acoustic_encoder_out["pos_wav2vec_out"][0].squeeze(-1).float(), 
                    target_pos,
                    weight=(~acoustic_encoder_out["padding_mask"][0]).float().transpose(0,1),
                    reduction = "sum"
                )
                # TODO align_loss * ntokens / nsentences
                #print(acoustic_encoder_out["pos_wav2vec_out"][0].shape, target_pos.shape, acoustic_encoder_out["padding_mask"][0].float().shape)
                loss =  loss  + word_loss + gama * align_loss
                if self.add_proj_norm:
                    loss += model.get_acoustic_projection_L2_norm(
                    reduction="sum")
            else:
                with torch.backends.cudnn.flags(enabled=False):
                    loss = F.ctc_loss(
        ...
        if self.contrastive_alpha > 0:
            ...
            if contrastive_beta > 1e-4:
                ...
                if self.use_token_contrastive:
                    embed_out, _ = model.textual_encoder.forward_embedding(reverse_sample["net_input"]["src_tokens"])
            ...
            if contrastive_beta < 1:
                ...
                if self.use_token_contrastive:
                    embed_out, _ = model.textual_encoder.forward_embedding(kd_sample["net_input"]["src_tokens"])
        ...
        if self.max_position_ctc != 0:
            logging_output["align_loss"] = align_loss 
        else:
            logging_output["align_loss"] = 0
        ...
        if not model.training:
            ...
            with torch.no_grad():
                ...
                for lp, wlp, t, inp_l in zip(
                    ...
                    #print(len(targ_words),tmp_toks.sum(),tmp_toks.tolist(),tmp_words.tolist())
                    #print(len(targ_words),tmp_toks.sum(),pred_units_arr,tmp_words.tolist())
    ...
    def reduce_metrics(logging_outputs) -> None:
        ...
        metrics.log_scalar(
            "align_loss",
            align_loss / nsentences / math.log(2),
            nsentences,
            round=3,
        )
