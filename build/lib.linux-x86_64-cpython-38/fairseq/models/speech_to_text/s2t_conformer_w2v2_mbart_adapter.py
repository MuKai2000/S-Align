...
from fairseq.models.wav2vec import (
    Wav2Vec2Model, Wav2VecCtc, Wav2Vec2Config, Adapter
)
...
@register_model("s2t_conformer_w2v2_mbart_adapter")
class S2TConformerModelW2V2MBARTAdapter(FairseqEncoderDecoderModel):
    ...
    def build_model(cls, args, task):
        ...
        if hasattr(args, 'tune_w2v_adapter'):
            assert not hasattr(args, 'tune_w2v_LNA')
            for key, value in encoder.wav2vec_model.named_parameters():
               if "layer_norm" in key:
                   value.requires_grad = True
               elif "adapter" in key:
                   ...
                   value.requires_grad = False
            logging.info("only tune the wav2vec layer normal and adapter module")
...
class S2T_W2V2_ConformerEncoder(S2TTransformerEncoder):
    ...
    def __init__(self, args, task=None, embed_tokens=None):
        ...
        with open_dict(self.w2v_args):
            ...
            self.w2v_args.adapter_scale = getattr(args, "adapter_scale", 0)
            self.w2v_args.adapter_dropout = getattr(args, "adapter_dropout", 0)
...
@register_model_architecture(model_name="s2t_conformer_w2v2_mbart_adapter",
                             arch_name="s2t_conformer_w2v2_mbart_adapter")
...
@register_model_architecture("s2t_conformer_w2v2_mbart_adapter", "s2t_conformer_w2v2_mbart_adapter_s")
...
@register_model_architecture("s2t_conformer_w2v2_mbart_adapter", "s2t_conformer_w2v2_mbart_adapter_sp")
