...
class S2TTransformerModelW2V2(FairseqEncoderDecoderModel):
    ...
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        return TransformerDecoderScriptable(args, tgt_dict, embed_tokens)
...
class S2T_W2V2_TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args):
        ...
        self.w2v_args = Wav2Vec2Config(state["cfg"])
        ...
        if not self.use_asr_finetune_w2v: # if use ssl-trained only
            ...
            if not self.reset_w2v:
                self.wav2vec_model.load_state_dict(state["model"],False)
            else:
                logger.info('not loading wave2vec pretrained weights')
        else: # wav2vec-ctc model
            # define the task!
            # change the data path
            ckpt["args"].data = args.data # todo: to check: if args.data exists
            ...
            assert not getattr(args, 'wav2vec_quasiwave', False)
...
@register_model_architecture(model_name="s2t_transformer_w2v2",
                             arch_name="s2t_transformer_w2v2")
...
@register_model_architecture("s2t_transformer_w2v2", "s2t_transformer_w2v2yr_s")
def s2t_transformer_w2v2yr_s(args):
