...
from fairseq.modules import LayerNorm, PositionalEmbedding, TransformerDecoderLayer, TransformerEncoderLayer, ConformerEncoderLayer,RelPositionalEncoding
...
@dataclass
class CmclAdapterConfig(FairseqDataclass):
    w2v_path: str = field(
        default=MISSING, metadata={"help": "path to wav2vec 2.0 model"}
    )
    mbart_path: str = field(
        default=MISSING, metadata={"help": "path to mbart model"}
    )
    mbart_filter_size: int = field(
        default=0, metadata={"help": "ffn filter size of mbart"}
    )
    embed_path: str = field(
        default="", metadata={"help": "path to word embed"}
    )
    embed_dim: int = field(
        default=1024, metadata={"help": "bart word embed size"}
    )
...
@register_model("cmcl_adapter", dataclass=CmclAdapterConfig)
class CmclAdapter(BaseFairseqModel):
    def __init__(self, cfg: CmclAdapterConfig, w2v_encoder: BaseFairseqModel, mbart_encoder: BaseFairseqModel):
        super().__init__()
        #import argparse
    ...
    @classmethod
    def build_model(cls, cfg: CmclAdapterConfig, task: FairseqTask):
        ...
        embed_tokens = build_embedding(
                task.target_dictionary, cfg.embed_dim, getattr(cfg, "embed_path", None)
                #task.target_dictionary, cfg.embed_dim, None
            )
        mbart_encoder= BartEncoder(cfg, task.target_dictionary, embed_tokens)
        if cfg.freeze_modules!="":
            ...
            for key, value in mbart_encoder.named_parameters():
               for freeze_name in freeze_names:
                   if freeze_name in "mbart_encoder."+key:
                       value.requires_grad = False
               if "down_proj" in "mbart_encoder."+key:
                   value.requires_grad = True
               if "up_proj" in "mbart_encoder."+key:
                   value.requires_grad = True
        if cfg.tune_w2v_LNA:
        ...
        return cls(cfg, w2v_encoder, mbart_encoder)
...
class NormalAdapter(BaseFairseqModel):
    def  __init__(self, cfg: CmclAdapterConfig, padding_idx):
        super().__init__()
        ...
        self.layers=nn.ModuleList(
            [ConformerEncoderLayer(cfg.encoder_embed_dim,
...
class Wav2vecEncoder(FairseqEncoder):
    def __init__(self, cfg: CmclAdapterConfig, tgt_dict=None):
        ...
        w2v_args.task.label_dir = cfg.data
        ...
        assert d==cfg.encoder_embed_dim,"the w2v size should be same with adapter size"

        if cfg.use_ctc_loss:
            if tgt_dict is not None:
                self.proj = Linear(d, len(tgt_dict), bias=False)
                if cfg.embed_path != "":
                    ...
                    self.proj.weight = load_embedding_weight(tgt_dict, d, cfg.embed_path)
        ...
        if "wav2vec_small" not in cfg.w2v_path and "hubert" not in cfg.w2v_path:
            logging.info("load pretrained big wav2vec")
...
class BartEncoder(TransformerEncoder):
    def __init__(self, cfg: CmclAdapterConfig, tgt_dict, embed_tokens):
        arg_overrides = {
            "dropout": cfg.dropout,
        }
        if cfg.mbart_args is None:
            state = checkpoint_utils.load_checkpoint_to_cpu(cfg.mbart_path, arg_overrides)
            mbart_args = state.get("cfg", None) 
            assert  mbart_args is not None,("Miss mbart args") 
        #arg=copy.deepcopy()
        #arg.decoder_layers=arg.mbart_layers
        #arg.input_embed_dim = embed_tokens.embedding_dim
        super().__init__(mbart_args.model, tgt_dict, embed_tokens)
        ...
        self.filter_size = cfg.mbart_filter_size
