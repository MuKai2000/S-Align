...
@register_model("s2t_conformer_w2v2_mbart")
class S2TConformerModelW2V2MBART(FairseqEncoderDecoderModel):
...
class S2T_W2V2_ConformerEncoder(S2TTransformerEncoder):
    ...
    def __init__(self, args, task=None, embed_tokens=None):
        ...
        if state["cfg"].model.w2v_args is None:
            #state = checkpoint_utils.load_checkpoint_to_cpu(state["cfg"].model.w2v_path, arg_overrides={})
            args_state = checkpoint_utils.load_checkpoint_to_cpu("/mnt/zhangyuhao/pretrain-model/w2v_large_lv_fsh_swbd_cv_ftls960_updated.pt", arg_overrides={})
            w2v_args = args_state.get("cfg", None)
            if w2v_args is None:
                w2v_args = convert_namespace_to_omegaconf(args_state["args"])
            self.w2v_args = w2v_args.model
        else:
        ...
        if not self.use_asr_finetune_w2v: # if use ssl-trained only
            ...
            if not self.reset_w2v:
                ...
                #else:
                #    state['model']['quantizer.vars'] = torch.randn(1, 640, 384)
                #    state['model']['quantizer.weight_proj.weight'] = torch.randn(640, 512)
                #    state['model']['quantizer.weight_proj.bias'] = torch.randn(640)
                #    state['model']['project_q.weight'] = torch.randn(768, 768)
                #    state['model']['project_q.bias'] = torch.randn(768)
                #    state['model']['final_proj.weight'] = torch.randn(768, 1024)
                #    state['model']['final_proj.bias'] = torch.randn(768)
                self.wav2vec_model.load_state_dict(state["model"],True)
                logging.info("load wav2vec2 model: {}".format(self.w2v2_model_path))
            else:
        ...
        #add adapter for mbart
        self.adapter = Adapter(
            w2v_output_dim,
            args.adapter_dim,
            args.adapter_dropout
        ) if args.adapter_dim else None
        ...
        #print(subsample_state)
        if subsample_state != {} and self.subsample is not None:
            self.subsample.load_state_dict(subsample_state, True)
            logger.info(f"load pretrained subsample")
        # self.subsample = Conv1dSubsampler(
        ...
        #)
        macaron_style=getattr(args, "macaron_style", False)
        if macaron_style:
        ...
        if args.encoder_layers != 0 and layers_state != {}:
            self.layers.load_state_dict(layers_state)
            logger.info(f"load pretrained layers")
    ...
    def forward(self, src_tokens, src_lengths, **extra_args):
        ...
        #adapter for mbart
        if self.adapter:
             w2v_feature = self.adapter(w2v_feature)
        ...
        x = self.embed_scale * conv_feature
        ...
        if not self.use_w2v_ctc:
            wav_feature=w2v_feature
...
class Adapter(nn.Module):
    ...
    """
    def __init__(self, feature_dim,  proj_dim, dropout):
        super().__init__()
        self.layer_norm = LayerNorm(feature_dim)
        self.down_proj = nn.Linear(feature_dim, proj_dim)
        self.up_proj = nn.Linear(proj_dim, feature_dim)
...
@register_model_architecture(model_name="s2t_conformer_w2v2_mbart",
                             arch_name="s2t_conformer_w2v2_mbart")
...
@register_model_architecture("s2t_conformer_w2v2_mbart", "s2t_conformer_w2v2_mbart_s")
...
@register_model_architecture("s2t_conformer_w2v2_mbart", "s2t_conformer_w2vyr_mbart_s")
def s2t_conformer_w2v2yr_s(args):
    s2t_conformer_w2v2_s(args)


@register_model_architecture("s2t_conformer_w2v2_mbart", "s2t_conformer_w2v2_mbart_sp")
...
@register_model_architecture("s2t_conformer_w2v2_mbart", "s2t_conformer_w2v2asr_mbart_s")
