...
@register_model("s2t_w2v2_mbart_sate")
class S2TW2V2MBARTSATE(FairseqEncoderDecoderModel):
    ...
    def build_encoder(cls, args, task=None, embed_tokens=None):
        encoder = S2T_W2V2_SATE_Encoder(args, task, embed_tokens)
...
class S2T_W2V2_SATE_Encoder(S2TTransformerEncoder):
    ...
    def __init__(self, args, task=None, embed_tokens=None):
        ...
        self.adapter = SateAdapter(args, task.source_dictionary, embed_tokens)
        if self.w2v_args.encoder_embed_dim != state["cfg"].model.encoder_embed_dim:
            d = self.w2v_args.encoder_embed_dim
        ...
        if getattr(args, "load_encoder_layers_from", None):
            ...
            layer_model_state = checkpoint_utils.load_checkpoint_to_cpu(args.load_encoder_layers_from)
            component_state_dict = OrderedDict()
            for key in layer_model_state["model"].keys():
                if key.startswith("encoder.layers"):
                    # encoder.input_layers.0.0.weight --> input_layers.0.0.weight
                    component_subkey = key[len("encoder.layers") + 1 :]
                    component_state_dict[component_subkey] = layer_model_state["model"][key]
    ...
    def forward(self, src_tokens, src_lengths, **extra_args):
        ...
        ctc_prob = self.compute_ctc_prob(x)
        x = (x, ctc_prob)
...
@register_model_architecture(model_name="s2t_w2v2_mbart_sate",
                             arch_name="s2t_w2v2_mbart_sate")
...
@register_model_architecture("s2t_w2v2_mbart_sate", "s2t_w2v2_mbart_sate_s")
def s2t_w2v2_mbart_sate_s(args):
