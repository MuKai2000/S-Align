...
def load_pretrained_component_from_model(
    ...
    for key in state["model"].keys():
        if key.startswith(component_type):
            ...
            if ("embed_tokens" in component_subkey or "output_projection" in component_subkey) and embed_tokens is not None:
                if not state["cfg"]["model"].share_decoder_input_output_embed and "output_projection" in component_subkey:
                    component_state_dict[component_subkey] = state["model"][key]
                else:
                    component_state_dict[component_subkey] = embed_tokens
            else:
                component_state_dict[component_subkey] = state["model"][key]
