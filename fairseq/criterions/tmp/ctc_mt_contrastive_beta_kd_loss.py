...
@dataclass
class CTCMTContrastiveBetaKDLossConfig(CtcCriterionConfig):
...
@register_criterion("ctc_mt_contrastive_beta_kd_loss", dataclass=CTCMTContrastiveBetaKDLossConfig)
class CTCMTContrastiveBetaKDLoss(CtcCriterion):
    def __init__(self, cfg:CTCMTContrastiveBetaKDLossConfig, task:FairseqTask): #, sentence_avg, contrastive_lambda=0.0, contrastive_temperature=1.0, get_similarity=False):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        mbart_out = model.mt_model_encoder(reverse_sample["net_input"]["src_tokens"], reverse_sample["net_input"]["src_lengths"])
        kd_sample = self.gen_kd_sample(sample, lprobs)
        kd_mbart_out = model.mt_model_encoder(kd_sample["net_input"]["src_tokens"], kd_sample["net_input"]["src_lengths"])
