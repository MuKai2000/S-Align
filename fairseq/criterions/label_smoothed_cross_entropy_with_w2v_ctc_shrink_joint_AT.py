...
class LabelSmoothedCrossEntropywithW2vCtcShrinkJointAT(
    ...
    def __init__(self, task, sentence_avg, label_smoothing, post_process="letter", ctc_weight=0.0, contrastive_alpha=0.0, 
        ...
        self.merge_mt_st = task.merge_mt_st
    ...
    def forward(self, model, sample, per_task, reduce=True):
        ...
        if per_task == "st":
            ...
            if self.merge_mt_st:
                # do mt task
                _, _, prev_output_tokens = sample["net_input"].values()
                src_tokens, src_lengths = sample["transcript"]['tokens'], sample["transcript"]['lengths']
                encoder_out = model.textual_encoder(src_tokens, src_lengths)
                net_output = model.decoder(
                    prev_output_tokens=prev_output_tokens, encoder_out=encoder_out
                )  
                mt_loss, mt_nll_loss = self.compute_loss(model, net_output, sample, reduce=reduce)
                mt_sample_size = (
                    sample["target"].size(0) if self.sentence_avg else sample["ntokens"]
                )

                logging_output["trans_loss"] += (utils.item(mt_loss.data) if reduce else mt_loss.data)
                logging_output["nll_loss"] += (utils.item(mt_nll_loss.data) if reduce else mt_nll_loss.data)
                logging_output["ntokens"] += sample["ntokens"]
                logging_output["nsentences"] += sample["target"].size(0)
                logging_output["sample_size"] += mt_sample_size
        ...
        # ************/ Add AT per-task /************ #
            ...
            assert self.at_training, 'BUG HERE: should not do AT task!'
            ...
            assert not self.merge_mt_st, 'BUG HERE: should not do MT task!'
