...
@register_criterion("label_smoothed_cross_entropy_with_double_ctc")
class LabelSmoothedCrossEntropyCriterionWithDoubleCTC(
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if self.ctc_weight > 0:
            ...
            logging_output["wv2_ctc_loss"] = utils.item(w2v_ctc_loss.data)
            loss = (1 - self.ctc_weight) * loss + self.ctc_weight * (0.5 * ctc_loss + 0.5 * w2v_ctc_loss)
            logging_output["loss"] = utils.item(loss.data) if reduce else loss.data

        return loss, sample_size, logging_output

    def compute_ctc_loss(self, model, sample, encoder_out, wav_ctc=False):
        transcript = sample["transcript"]
        if wav_ctc:
            ctc_logit = model.encoder.compute_w2v_ctc_logit(encoder_out) 
        else:
