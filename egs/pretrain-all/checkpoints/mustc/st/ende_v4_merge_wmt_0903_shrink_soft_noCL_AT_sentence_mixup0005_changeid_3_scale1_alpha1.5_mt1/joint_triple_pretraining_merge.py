...
class JointTriplePretrainingMergeTask(SpeechToTextTask):
    ...
    def _per_task_train_loss(
        ...
        if self.merge_mt_st and per_task == 'st':       # Task合并的ST 这里有(AT,)MT,ST(三)两项任务
            ...
            mixup_change_id_scale = 3
            if self.mixup_change_id:
                loss_at = loss_at * mixup_change_id_scale
            if update_num < 1000 * (mixup_change_id_scale if self.mixup_change_id else 1):   # 前1k步不更新gen任务
            ...
            if self.at_training: #  and not ignore_grad:
                ...
                if update_num == 2000 * (mixup_change_id_scale if self.mixup_change_id else 1):
                    print("--------------------")
            ...
            if update_num > 35000 and (update_num % 2000) == 0:
                print(f"--Backword ST Loss", loss_st, "\tMT Loss", loss_mt)
