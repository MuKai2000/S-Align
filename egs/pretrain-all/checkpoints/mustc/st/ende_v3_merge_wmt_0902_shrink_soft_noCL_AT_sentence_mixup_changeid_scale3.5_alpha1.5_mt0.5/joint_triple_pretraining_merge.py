...
class JointTriplePretrainingMergeTask(SpeechToTextTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            ...
            default=False,
            type=bool,
            help="change target id when doing mixup",
        )
        parser.add_argument(
            "--mix-tag",
            default=0.5,
            type=float,
            help="generator tag for mix features",
        )
        parser.add_argument(
            "--mixup-for-whole-model",
            default=False,
            type=bool,
            help="use mixup for whole model when it's TRUE, or only AT network when it's FALSE",
        )
        parser.add_argument(
            "--at-level",
            default="sentence",
    ...
    def __init__(self, args, tgt_dict, src_dict=None, cluster_convert=None):
        ...
        self.at_low_pos = args.at_low_pos
        self.at_adapte_win = args.at_adapte_win
        self.at_nopad = args.at_nopad
        self.at_nomute = args.at_nomute
        self.mix_tag = args.mix_tag
        self.mixup_rate = args.mixup_rate
        self.mixup_change_id = args.mixup_change_id
        self.mixup_for_whole_model = args.mixup_for_whole_model
        # ************/ Add Task set /************ #
    ...
    def _per_task_train_loss(
        ...
        if self.merge_mt_st and per_task == 'st':       # Task合并的ST 这里有(AT,)MT,ST(三)两项任务
            ...
            if ignore_grad:     # 前3k步不更新st任务
                loss_st = loss_st * 0
            if update_num < 1000:   # 前1k步不更新gen任务
            ...
            if self.at_training: #  and not ignore_grad:
                ...
                if update_num > 6000 and False:
                    limit_scale = 5.0
                    if abs(loss_at.data) >  (limit_scale * abs(loss_at_gen.data)):
                        loss_at_gen = loss_at_gen * 0
                        loss_at = loss_at / limit_scale
                    if abs(loss_at_gen.data) >  (limit_scale * abs(loss_at.data)):
                        loss_at = loss_at * 0
                        loss_at_gen = loss_at_gen / limit_scale

                #loss_at = -1 * loss_at
                loss_at.backward(retain_graph=True) 
                for name, param in model.named_parameters():
                    if param.grad is not None:
                        if 'task_net' not in name:
                            print("not task_net", name, param.requires_grad, param.grad[:5])
                ...
                """for name, param in model.task_net.named_parameters():
                    if param.grad is not None:
                        param.grad = -1 * param.grad"""
                
                if update_num == 2000:
                    print("--------------------")
                    print("Before gen at loss:")
                    for name, param in model.textual_encoder.named_parameters():
                        if param.grad is not None:
                            print("textual_encoder", name, param.requires_grad, param.grad[:5])
                            break
                    for name, param in model.task_net.named_parameters():
                        if param.grad is not None:
                            print("task_net", name, param.requires_grad, param.grad[:5])
                ...
                loss_at_gen.backward(retain_graph=True)
                # unfreeze
                for name, param in model.task_net.named_parameters():
                    param.requires_grad = True
                
                if update_num == 2000:
                    print("After gen at loss:")
                    for name, param in model.textual_encoder.named_parameters():
                        if param.grad is not None:
                            print("textual_encoder", name, param.requires_grad, param.grad[:5])
                            break
            ...
            loss_mt = self.mt_weight * loss_mt
            loss_mt_st = loss_st + loss_mt # note zyh change here 08.01.2023
            ...
            loss = ((loss_at + loss_at_gen) if loss_at else torch.tensor(0.0)) + (loss_st if loss_st else torch.tensor(0.0)) + (loss_mt if loss_mt else torch.tensor(0.0))
    ...
    def save_top_output(self, models, sample):
        ...
        current_directory = '/workspace/fairseq-AT/egs/pretrain-all'
    ...
    def inference_step(
        ...
        if False:
            if (sample['transcript']['tokens'] is not None) and ('results_path' in dir(self.args)):       # //need_check// 
                print("Save Top output")     
                with torch.no_grad():  
