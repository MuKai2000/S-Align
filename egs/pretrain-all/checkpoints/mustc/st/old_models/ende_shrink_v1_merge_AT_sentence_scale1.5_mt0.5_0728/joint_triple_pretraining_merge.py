...
class JointTriplePretrainingMergeTask(SpeechToTextTask):
    ...
    def __init__(self, args, tgt_dict, src_dict=None, cluster_convert=None):
        ...
        self.mt_weight = torch.tensor(0.5)          # //need_check//              
        logger.info(
    ...
    def save_top_output(self, models, sample, nomute=False, nopad=False):
        ...
        st_encoder_padding_mask = st_encoder_out["encoder_padding_mask"][0] if nomute else torch.zeros(st_encoder_out["encoder_padding_mask"][0].shape).bool().to(st_encoder_out["encoder_padding_mask"][0].device)    # [bs, len]
        mt_embedding = mt_encoder_out["encoder_embedding"][0]
        mt_encoder_output = mt_encoder_out["encoder_out"][0].transpose(0,1)                    # [bs, len, dim]
        mt_encoder_padding_mask= lengths_to_padding_mask(mt_src_lengths) if nopad else torch.zeros(mt_encoder_out["encoder_padding_mask"][0].shape).bool().to(mt_encoder_out["encoder_padding_mask"][0].device)    # [bs, len]
