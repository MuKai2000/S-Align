        ...
        if per_task == 'st' or (per_task == 'at' and sample['st'] is not None):            # sample_st --> acoustic_encoder --> encoder_out_st
            ...
            if self.at_training and (per_task == 'at' or multi_st):
                ...
                if self.at_level == "sentence":
                    ...
                    tgt_len = sample_st["transcript"]['tokens'].shape[-1]
                    _shape = encoder_output.shape # [bs, len, dim]
                    # print(" > 1. ", _shape, st_encoder_output.dtype)
                    length = _shape[1]
                    
                    if self.at_adapte_win:      # let fin speech len equals to text len
                        assert not self.at_adapte_win, "at_adapte_win feature has been delete, please use fix window set"
                        """num_win = tgt_len
                        # set windows
                        self.at_win['size'] = int((length // num_win) * 1.25)
                        self.at_win['stride'] = int((length-self.at_win['size'])//(num_win-1)) + (0 if (length-self.at_win['size']) % (num_win-1)==0 else 1)"""
                    else:                       # set a fix window, detail in self.__ini__()
        ...
        if per_task == 'mt' or (per_task == 'at' and sample['mt'] is not None) or multi_st:            # sample_mt --> textual_encoder --> encoder_out_mt
            ...
            if self.at_training and (per_task == 'at' or multi_st):
                ...
                if self.at_level == "sentence":
                    ...
                    id_mt = torch.zeros(prob_mt.shape[0], 1)
                else:
                    raise NotImplementedError
