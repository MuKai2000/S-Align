...
class LabelSmoothedCrossEntropywithW2vCtcShrinkJointAT(
    ...
    def __init__(self, task, sentence_avg, label_smoothing, post_process="letter", ctc_weight=0.0, contrastive_alpha=0.0, 
        ...
        self.at_win = {'size':50, 'stride':10}     # works when "at_level" is "window"                      # //need_check//
    ...
    def forward(self, model, sample, per_task, reduce=True):
        ...
        # ************/ Add AT per-task /************ #
            ...
            st_lengths, st_prob, st_id = None, None, None
            mt_lengths, mt_prob, mt_id = None, None, None
            # st
            if sample['st'] is not None:
                ...
                st_encoder_padding_mask = st_encoder_out["encoder_padding_mask"][0]     # [bs, len]
                if self.at_level == "sentence":
                    ...
                    if self.at_nopad:   # 去除padding mask部分
                    ...
                    # 计算窗口个数
                    clean_encoder_output = st_encoder_output.transpose(0,1)[st_encoder_padding_mask.unsqueeze(-1)] = 0.0    # 将padding位置设为0，避免影响mean运算
                    _shape = clean_encoder_output.shape # [bs, len, dim]
                    length = _shape[0]
                    num_win = int((length - self.at_win['size']) / self.at_win['stride']) + 1
                    task_input = torch.zeros(_shape[1], num_win, _shape[-1])  # [bs, num_win, dim]
            ...
            if sample['mt'] is not None:
                ...
                mt_encoder_padding_mask = mt_encoder_out["encoder_padding_mask"][0]     # [bs, len]
                if self.at_level == "sentence":
                    ...
                    if self.at_nopad:   # 去除padding mask部分
                        _shapeA = encoder_output.shape  # record shape
                        encoder_padding_mask = mt_encoder_padding_mask.reshape(-1)  # [bs * len]
                        encoder_output = encoder_output[~encoder_padding_mask]      # 去除mask为True
                        _shapeB = encoder_output.shape  # record shape
                        if _shapeA != _shapeB:
                            print(_shapeA, _shapeB)
            ...
            if self.at_level == "sentence":
                if self.at_nopad:
                    weight = torch.cat((() if sample['st'] is None else ((~st_encoder_padding_mask).long().sum(dim=1),)) 
                ...
                print("> ", weight.shape, task_prob.shape)
                sentence_sample_size = task_prob.shape[0]          # sentence_sample_size = torch.sum(scale).item()
                # merge result
                ...
                weight = 1.0 / ((0 if sample['st'] is None else st_encoder_out["encoder_padding_mask"][0].shape[0])
                              + (0 if sample['mt'] is None else mt_encoder_out["encoder_padding_mask"][0].shape[0]))
                # bce loss
                bce_loss = F.binary_cross_entropy_with_logits(
                        task_prob.float(),
                        task_id,
