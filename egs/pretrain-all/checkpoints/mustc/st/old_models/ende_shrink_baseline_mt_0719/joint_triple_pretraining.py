...
class JointTriplePretrainingTask(SpeechToTextTask):
        ...
        for idx, per_task in enumerate(sample.keys()): #enumerate(["st","asr","mt"]):
            if per_task not in sample.keys():
                ...
                if per_task == "asr" and (update_num > 30000 or self.asr_weight < 0.1): # 30000 # 100000 asronly            //need_check//
                    continue
                if per_task == "mt" and (update_num > 100000 or self.mt_weight < 0.1): # 50000 # 100000 ende_shrink_AT_mt    //need_check//
                    continue
    ...
    def save_top_output(self, models, sample, nomute=False, nopad=False):
        ...
        temp_dir = '/workspace/fairseq-AT/egs/pretrain-all/pic/temp'
        ...
        st_embedding = st_encoder_out["wav_feature"][0]
        ...
        if op.isfile(spe_temp_file):
            ...
            # print("Speech:\t", data.shape, st_encoder_output.shape)
            st_encoder_output = torch.cat((st_encoder_output, data), dim=0)
        torch.save(st_encoder_output.cpu(), spe_temp_file)

        if op.isfile(txt_temp_file):
            data = torch.load(txt_temp_file)
            # print("Speech:\t", data.shape, mt_encoder_output.shape)
            mt_encoder_output = torch.cat((mt_encoder_output, data), dim=0)
        torch.save(mt_encoder_output.cpu(), txt_temp_file)
        # assert False
