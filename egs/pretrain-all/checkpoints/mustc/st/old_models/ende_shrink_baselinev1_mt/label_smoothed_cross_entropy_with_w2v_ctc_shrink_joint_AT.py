    ...
    def forward(self, model, sample, per_task, reduce=True):
        ...
        # ************/ Add AT per-task /************ #
            ...
            if sample['st'] is not None:
                ...
                st_encoder_padding_mask = st_encoder_out["encoder_padding_mask"][0] if self.at_nomute else torch.zeros(st_encoder_out["encoder_padding_mask"][0].shape).bool().to(st_encoder_out["encoder_padding_mask"][0].device)     # [bs, len]
                if self.at_level == "sentence":
                    st_prob = model.task_net(st_encoder_output, st_encoder_padding_mask)
            ...
            if sample['mt'] is not None:
                ...
                mt_encoder_padding_mask = lengths_to_padding_mask(mt_src_lengths) if self.at_nopad else torch.zeros(mt_encoder_out["encoder_padding_mask"][0].shape).bool().to(mt_encoder_out["encoder_padding_mask"][0].device)  # [bs, len]

                if self.at_level == "sentence":
                    mt_prob = model.task_net(mt_encoder_output, mt_encoder_padding_mask)
