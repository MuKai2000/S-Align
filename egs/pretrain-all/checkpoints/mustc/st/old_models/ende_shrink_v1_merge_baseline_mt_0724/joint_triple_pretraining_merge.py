...
class JointTriplePretrainingMergeTask(SpeechToTextTask):
    ...
    def __init__(self, args, tgt_dict, src_dict=None, cluster_convert=None):
        ...
        self.mt_weight = torch.tensor(1.0)
        logger.info(
    ...
    def _per_task_train_loss(
        ...
        # ************/ Merge ST&MT&AT Task /************ #
        if self.merge_mt_st and per_task == 'st':       # Task合并的ST 这里有(AT,)MT,ST三项任务
            # print(type(criterion))
            loss_at, loss_st, loss_mt, sample_size, logging_output = criterion(
                model, sample[per_task], per_task, merge_task=True
            )
    ...
    def train_step(
        ...
        # print(f"org per task list:{sample.keys()}")
        # ************/ Merge ST&MT&AT Task /************ # 
        task_list = list(sample.keys())      
        if self.merge_mt_st:                        # 合并的话去掉MT的索引
            task_list = ['st', 'asr']
        # print(f"per task list:{task_list}")
        # assert False
    ...
    def valid_step(self, sample, model, criterion):
        ...
        with torch.no_grad():
            ...
            # ************/ Add AT per-task /************ #
            if 'st' in sample.keys() and self.at_training and not self.merge_mt_st:
                # sample["at"] = {'st':sample['st'] if 'st' in sample.keys() else None, 'mt':sample['mt'] if 'mt' in sample.keys() else None} # add at sample
                sample["at"] = {'st':sample['st'], 'mt': None}
            
            for idx, per_task in enumerate((self.all_tasks + ['at']) if ('st' in sample.keys() and self.at_training and not self.merge_mt_st) else self.all_tasks):
                if (
    ...
    def save_top_output(self, models, sample, nomute=False, nopad=False):

        assert False

        # set pic name
        # exp_name = "ende_shrink_AT_mt"
        exp_name = self.args.save_dir.split('/')[-1]
        level = 'sentence'
        ...
        # model output
        st_embedding = st_encoder_out["wav2vec_feature"][0].transpose(0,1)
        st_encoder_output = st_encoder_out["encoder_out"][0].transpose(0,1)                    # [bs, len, dim]
        st_encoder_padding_mask = st_encoder_out["encoder_padding_mask"][0] if nomute else torch.zeros(st_encoder_out["encoder_padding_mask"][0].shape).bool().cuda()    # [bs, len]
        mt_embedding = mt_encoder_out["encoder_embedding"][0]
        mt_encoder_output = mt_encoder_out["encoder_out"][0].transpose(0,1)                    # [bs, len, dim]
        mt_encoder_padding_mask= lengths_to_padding_mask(mt_src_lengths) if nopad else torch.zeros(mt_encoder_out["encoder_padding_mask"][0].shape).bool().cuda()       # [bs, len]
        
        # print(st_encoder_padding_mask.shape, mt_encoder_padding_mask.shape)
        # print(st_encoder_output.shape, st_embedding.shape)
        # print(mt_encoder_output.shape, mt_embedding.shape)
        
        # assert False
        # preprocess
        if level == 'sentence':
            st_embedding[st_encoder_padding_mask.unsqueeze(-1).expand(st_encoder_output.shape)] = 0.0
            st_embedding = st_embedding.sum(dim=1) / (~st_encoder_padding_mask).long().sum(dim=1).unsqueeze(-1)   # [bs, dim]
            st_encoder_output[st_encoder_padding_mask.unsqueeze(-1).expand(st_encoder_output.shape)] = 0.0
            st_encoder_output = st_encoder_output.sum(dim=1) / (~st_encoder_padding_mask).long().sum(dim=1).unsqueeze(-1)   # [bs, dim]
            
            mt_embedding[mt_encoder_padding_mask.unsqueeze(-1).expand(mt_encoder_output.shape)] = 0.0
            mt_embedding = mt_embedding.sum(dim=1) / (~mt_encoder_padding_mask).long().sum(dim=1).unsqueeze(-1)   # [bs, dim]
            mt_encoder_output[mt_encoder_padding_mask.unsqueeze(-1).expand(mt_encoder_output.shape)] = 0.0
            mt_encoder_output = mt_encoder_output.sum(dim=1) / (~mt_encoder_padding_mask).long().sum(dim=1).unsqueeze(-1)   # [bs, dim]
        ...
        if op.isfile(spe_temp_file):
            ...
            st_embedding = torch.cat((st_embedding.cpu(), data['emb']), dim=0)
            st_encoder_output = torch.cat((st_encoder_output.cpu(), data['output']), dim=0)
            
        torch.save({'output':st_encoder_output.cpu(), 'emb':st_embedding.cpu()}, spe_temp_file)

        if op.isfile(txt_temp_file):
            data = torch.load(txt_temp_file)
            mt_embedding = torch.cat((mt_embedding.cpu(), data['dim']), dim=0)
            mt_encoder_output = torch.cat((mt_encoder_output.cpu(), data['output']), dim=0)

        torch.save({'output':mt_encoder_output.cpu(), 'emb':mt_embedding.cpu()}, txt_temp_file)

        print("Name:\t{}\nSave:\t{}\t{}".format(exp_name, spe_temp_file, txt_temp_file))
        # assert False

    def inference_step(
        self, generator, models, sample, prefix_tokens=None, constraints=None, save_top=False
    ):
        if save_top:                # //need_check//
