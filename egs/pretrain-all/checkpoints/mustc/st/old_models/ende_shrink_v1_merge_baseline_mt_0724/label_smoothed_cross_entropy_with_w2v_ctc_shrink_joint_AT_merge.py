...
class LabelSmoothedCrossEntropywithW2vCtcShrinkJointATMerge(
    ...
    def forward(self, model, sample, per_task, reduce=True, merge_task=False):
        ...
        multi_st = (per_task == 'st' and merge_task)            # merge st task
        ...
        if per_task == 'st' or (per_task == 'at' and sample['st'] is not None):            # sample_st --> acoustic_encoder --> encoder_out_st
            ...
            src_tokens_st, src_lengths_st, prev_output_tokens_st = sample_st["net_input"].values()
            encoder_out_st = model.acoustic_encoder(src_tokens_st, src_lengths_st)
            ...
            if self.at_training and (per_task == 'at' or multi_st):                    # Record AT data for ST
                # print("-- Record AT data for ST", per_task)
                # required data
                encoder_output_st = encoder_out_st["encoder_out"][0]                    # [len, bs, dim]
                ...
                if self.at_level == "sentence":
                    prob_st = model.task_net(encoder_output_st, encoder_padding_mask_st if self.at_nomute else None)
                    id_st = torch.zeros(encoder_padding_mask_st.shape[0], 1)
                    ...
                    id_st = torch.zeros(prob_st.shape[0], 1)                        # [len*bs, 1]
                elif self.at_level == "window":
                    # 计算窗口个数
                    encoder_output = encoder_output_st.transpose(0,1)   # [bs, len, dim]
                    encoder_output[encoder_padding_mask_st.unsqueeze(-1).expand(encoder_output.shape)] = 0.0    # 将padding位置设为0，避免影响mean运算
                    ...
                    task_input = torch.zeros(num_win, _shape[0], _shape[-1], dtype=torch.float16).to(encoder_output.device)  # [num_win, bs, dim]  保存结果
                    ...
                    prob_st = model.task_net(task_input.unsqueeze(0))           # [1, num_unzero, dim=512] --> [num_unzero, 1]
                    id_st = torch.zeros(prob_st.shape[0], 1)                    # [num_unzero, 1]
                else:
        ...
        if per_task == 'mt' or (per_task == 'at' and sample['mt'] is not None) or multi_st:            # sample_mt --> textual_encoder --> encoder_out_mt
            ...
            if per_task == 'mt' or merge_task:     # Cal MT Loss: encoder_out_mt --> decoder --> net_output_mt
            ...
            if self.at_training and (per_task == 'at' or multi_st):            # Record AT data for MT
                ...
                if self.at_level == "sentence":
                    ...
                    id_mt = torch.ones(encoder_padding_mask_mt.shape[0], 1)
                    ...
                    id_mt = torch.ones(prob_mt.shape[0], 1)                         # [len*bs, 1]
                else:
                    raise NotImplementedError

        if self.at_training and (per_task == 'at' or multi_st):                # Cal AT Loss
            ...
            if self.at_level == "sentence":
                ...
                weight = torch.cat(
                          ...
                          + ((torch.ones(prob_mt.shape[0], dtype=torch.float16),) if ((per_task == 'at' and sample['mt'] is not None) or multi_st) else ())
            ...
            loss_at = loss_at * self.at_scale       # set scale for at loss

            logging_output["trans_loss"] += (utils.item(loss_at.data) if reduce else loss_at.data)
            ...
            loss = loss_at if loss is None else loss + loss_at
        ...
        if multi_st:
            return loss_at, loss_st, loss_mt, sample_size, logging_output
        else:
