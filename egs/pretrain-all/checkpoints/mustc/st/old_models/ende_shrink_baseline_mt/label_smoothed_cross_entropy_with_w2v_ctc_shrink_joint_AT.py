...
class LabelSmoothedCrossEntropywithW2vCtcShrinkJointAT(
    ...
    def __init__(self, task, sentence_avg, label_smoothing, post_process="letter", ctc_weight=0.0, contrastive_alpha=0.0, 
        ...
        self.at_winsize = 5     # works when "at_level" is "window"
        self.embedding_l2norm = task.embedding_l2norm
        assert (self.at_level == "sentence" or self.at_level == "token" or self.at_level == "window") , "Args: --at-level is wrong, expect \"sentence\" or \"token\"."
    ...
    def forward(self, model, sample, per_task, reduce=True):
        ...
        # ************/ Add AT per-task /************ #
            ...
            st_prob, st_id, mt_prob, mt_id = None, None, None, None
            # st
            if sample['st'] is not None:
                st_src_tokens, st_src_lengths, st_prev_output_tokens = sample['st']["net_input"]['src_tokens'], sample['st']["net_input"]['src_lengths'], sample['st']["net_input"]['prev_output_tokens']
                st_encoder_out = model.acoustic_encoder(st_src_tokens, st_src_lengths)
                if self.at_level == "sentence":
                    st_prob = model.task_net(st_encoder_out["encoder_out"][0])
                    st_id = torch.zeros(st_encoder_out["encoder_padding_mask"][0].shape[0], 1)
                elif self.at_level == "token":
                    st_prob = model.task_net(st_encoder_out["encoder_out"][0].reshape(-1, st_encoder_out["encoder_out"][0].shape[-1]).unsqueeze(0))
                    st_id = torch.zeros(st_prob.shape[0], 1)
                elif self.at_level == "window":
                    # n_wins = st_encoder_out["encoder_padding_mask"][0].shape[1]
                    pass
                elif self.at_level == "sentence_new":
                    encoder_out, encoder_padding_mask = st_encoder_out["encoder_out"][0], st_encoder_out["encoder_padding_mask"][0]
                    
                    pass

            # mt
            if sample['mt'] is not None:
                mt_src_tokens, mt_src_lengths, mt_prev_output_tokens = sample['mt']["net_input"]['src_tokens'], sample['mt']["net_input"]['src_lengths'], sample['mt']["net_input"]['prev_output_tokens']
                mt_encoder_out = model.textual_encoder(mt_src_tokens, mt_src_lengths)
                if self.at_level == "sentence":
                    mt_prob = model.task_net(mt_encoder_out["encoder_out"][0])
                    mt_id = torch.ones(mt_encoder_out["encoder_padding_mask"][0].shape[0], 1)
                elif self.at_level == "token" or self.at_level == "window":
                    mt_prob = model.task_net(mt_encoder_out["encoder_out"][0].reshape(-1, mt_encoder_out["encoder_out"][0].shape[-1]).unsqueeze(0))
                    mt_id = torch.ones(mt_prob.shape[0], 1)
            
            """print("###############", self.at_level)
            print(sample['st']["target"].size(0) if self.sentence_avg else sample['st']["ntokens"])
            print(sample['st']["target"].size(0), sample['st']["ntokens"])
            print(torch.sum(sample['st']["target_lengths"]))
            print(st_encoder_out["encoder_out"][0].reshape(-1, st_encoder_out["encoder_out"][0].shape[-1]).shape[0])"""
            ...
            if self.at_level == "sentence":
                ...
                # print("##################", st_encoder_out["encoder_out"][0].shape, sample['st']["target_lengths"])
                
                # scale
                # scale = torch.cat((() if sample['st'] is None else (torch.fill(st_encoder_out["encoder_out"][0].shape[0], st_encoder_out["encoder_out"][0].shape[1]),)) + (() if sample['mt'] is None else (torch.fill(mt_encoder_out["encoder_out"][0].shape[0], mt_encoder_out["encoder_out"][0].shape[1]),)), dim=0).unsqueeze(1)
                scale = torch.cat((() if sample['st'] is None else (sample['st']["target_lengths"],)) + (() if sample['mt'] is None else (sample['mt']["target_lengths"],)), dim=0).unsqueeze(1)
                ...
                sample_size = torch.sum(scale).item()
            
            elif self.at_level == "token":
                # weight
                weight = 1.0 / ((0 if sample['st'] is None else st_encoder_out["encoder_padding_mask"][0].shape[0]) + (0 if sample['mt'] is None else mt_encoder_out["encoder_padding_mask"][0].shape[0]))
                # bce loss
                loss = F.binary_cross_entropy_with_logits(
                        task_prob.float(),
                        task_id,
                        weight=torch.full(task_prob.shape, weight).to(task_prob.device),
                        reduction = "sum"
                )
                sample_size = task_prob.shape[0]
                # print("###############", st_prob.shape, st_id.shape, mt_prob.shape, mt_id.shape)
            ...
            # print("###############", utils.item(loss.data), loss.data, reduce, self.sentence_avg, sample_size)
            # assert False
