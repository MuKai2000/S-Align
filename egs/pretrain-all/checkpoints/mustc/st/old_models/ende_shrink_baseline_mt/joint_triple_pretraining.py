...
class JointTriplePretrainingTask(SpeechToTextTask):
    ...
    def _per_task_train_loss(
        ...
        #    ignore_grad=True
        if per_task == "st" and update_num < 3000:        # asronly 不做ctc_shrink去掉
            ignore_grad=True
            pass
    ...
    def train_step(
        ...
        if ('st' in sample.keys() or 'mt' in sample.keys()) and self.at_training is True:
            ...
            # sample.move_to_end('st', False) # change st task first for test
        ...
        for idx, per_task in enumerate(sample.keys()): #enumerate(["st","asr","mt"]): #
            if per_task not in sample.keys():
                ...
                if per_task == "asr" and (update_num > 30000 or self.asr_weight < 0.1): # 30000 # 100000 asronly 
                    continue
                if per_task == "mt" and (update_num > 100000 or self.mt_weight < 0.1): # 50000 # 100000 ende_shrink_AT_mt
                    continue
        ...
        if update_num % self.weight_steps == 0 and update_num != 0 :
            ...
            if torch.distributed.is_initialized():
                ...
                if self.mt_weight >= 0.1 and update_num < 50000:
                    ...
                    # self.mt_weight = tmp_mt_weight / world_size       # ende_shrink_AT_mt 不更新mt权重
