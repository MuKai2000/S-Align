        ...
        # ************/ Add AT per-task /************ #
            ...
            if sample['st'] is not None:
                ...
                if self.at_level == "sentence":
                    ...
                    if self.at_nomute:   # 去除padding mask部分
            ...
            if sample['mt'] is not None:
                ...
                mt_encoder_padding_mask= lengths_to_padding_mask(mt_src_lengths)
            ...
            if self.at_level == "sentence":
                weight = torch.cat(
                        (() if sample['st'] is None else (((~st_encoder_padding_mask).long().sum(dim=1)) if self.at_nomute else (torch.ones(st_encoder_padding_mask.shape).sum(dim=1).to(task_prob.device)),))
                        + (() if sample['mt'] is None else (((~mt_encoder_padding_mask).long().sum(dim=1)) if self.at_nopad else (torch.ones(mt_encoder_padding_mask.shape).sum(dim=1).to(task_prob.device)),)),
                        dim=0
