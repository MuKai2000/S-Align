    ...
    def forward(self, src_tokens, src_lengths, tbc=True, mixup_rate=0.0, textual_encoder=None, update_num=None, **kwargs):
        ...
        if self.use_ctc_shrink and ft:
            ...
            if mixup_rate != 0.0 and tokens_after_shrink is not None:                                          # //need_check//
                ...
                mixup_mask = mixup_mask & mixup_sent_mask
                # indices_to_replace = mixup_mask.nonzero(as_tuple=True)
                x = x.transpose(0, 1)           # [bs, len, dim]
                
                if update_num and update_num % 5000 == 0:
                    print(f"Mixup rate:{mixup_rate}, token after shrink shape:{tokens_after_shrink.shape}, X shape:{x.shape}")
                    print(f"CTC Tokens:{tokens[1][:5]}, Shrink Mask:{shrink_mask[1][:5]}, New Tokens:{tokens_after_shrink[1][:5]}\nOrg X:{x[1][:5]}")      
                
                x[mixup_mask] = text_embedding[mixup_mask]
                
                if update_num and update_num % 5000 == 0:
                    print(f"Mixup Mask:{mixup_mask}, Mixup Sent Mask:{mixup_sent_mask[:10, 0, 0]}, New X:{x[1][:5]}")
                
                x = x.transpose(0, 1)           # [len, bs, dim]
