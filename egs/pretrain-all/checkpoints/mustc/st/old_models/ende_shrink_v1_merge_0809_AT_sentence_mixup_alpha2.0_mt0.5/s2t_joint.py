        ...
        if self.use_ctc_shrink and ft:
            ...
            if mixup_rate != 0.0 and tokens_after_shrink is not None :                                          # //need_check//
                ...
                if torch.rand(1) < mixup_sent_rate:         # do for whole batch
                    x_mixup = x.clone()
                    if mixup_rate < 0:
                        gen_mixup_rate = 0.6 * torch.rand(tokens_after_shrink.shape[0]) + 0.2    # [bs]
                        gen_mixup_rate = gen_mixup_rate.unsqueeze(-1)
                    # else:
                    #     gen_mixup_rate = torch.full(tokens_after_shrink.shape[0], mixup_rate)          # note kkq change here 08.09.2023
                    #     gen_mixup_rate = gen_mixup_rate.unsqueeze(-1)
