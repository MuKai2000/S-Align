...
class AcousticEncoder(Wav2vecEncoder):
    ...
    def forward(self, src_tokens, src_lengths, tbc=True, mixup_rate=0.0, textual_encoder=None, update_num=None, **kwargs):
        ...
        if self.use_ctc_shrink and ft:
            ...
            padding_mask = real_padding_mask.new_zeros(padding_mask.shape).bool()                # //need_check//
            ...
            if mixup_rate != 0.0 and tokens_after_shrink is not None:                                          # //need_check//
                # print(tokens.shape, shrink_mask.shape, x.shape)
                # tokens_after_shrink = tokens.new_zeros(shrink_mask.shape[0], lengths.max())   # [bs, len]
                # print(tokens[1], shrink_mask[1], tokens[1][shrink_mask[1]], tokens_after_shrink[1])
                # for i, v in enumerate(lengths):
                #     tokens_after_shrink[i, : v] = tokens[i][shrink_mask[i]][:]
                # print(x.shape, tokens_after_shrink.shape, tokens_after_shrink[1])
                # print("Before shrink: ", wav_feature.shape)
                # torch.manual_seed(int(time.time() * 10000))
                x_mixup = x.clone()
                mixup_sent_rate = 0.3
                mixup_sent_mask = torch.rand(tokens_after_shrink.shape[0]).to(tokens_after_shrink.device) < mixup_sent_rate        # [bs]
                mixup_sent_index = mixup_sent_mask.nonzero(as_tuple=False)
                print_index = None if mixup_sent_index.numel() == 0 else mixup_sent_index[0].item()
                if mixup_rate < 0:
                    mixup_rate = 0.4 * torch.rand() + 0.2             # 0.2 ~ 0.6
                mixup_mask = torch.rand(tokens_after_shrink.shape).to(tokens_after_shrink.device) < mixup_rate      # [bs, len]
                _, text_embedding = textual_encoder.forward_embedding(tokens_after_shrink)
                mixup_mask = mixup_mask.unsqueeze(-1).expand(text_embedding.shape)  # [bs, len, dim] 
                mixup_sent_mask = mixup_sent_mask.unsqueeze(-1).expand(tokens_after_shrink.shape).unsqueeze(-1).expand(text_embedding.shape)    # [bs, len, dim]
                mixup_mask = mixup_mask & mixup_sent_mask   # [bs, len, dim]
                # indices_to_replace = mixup_mask.nonzero(as_tuple=True)
                x_mixup = x_mixup.transpose(0, 1)           # [bs, len, dim]
                
                if update_num and update_num % 5000 == 0 and print_index:
                    print(f"Mixup rate:{mixup_rate}, token after shrink shape:{tokens_after_shrink.shape}, X shape:{x_mixup.shape}")
                    print(f"CTC Tokens:{tokens[print_index,:5]}, Shrink Mask:{shrink_mask[print_index,:5]}, New Tokens:{tokens_after_shrink[print_index,:5]}")      
                
                x_mixup[mixup_mask] = text_embedding[mixup_mask]
                
                if update_num and update_num % 5000 == 0 and print_index:
                    print(f"Mixup Sent Mask:{mixup_sent_index}, {print_index},  Mixup Mask:{mixup_mask[print_index,:5,0]}, \n\
                    Org X:{x.transpose(0, 1)[print_index,:5]}, New X:{x_mixup[print_index,:5]}, Emb: {text_embedding[print_index,:5]}")
                
                x_mixup = x_mixup.transpose(0, 1)           # [len, bs, dim]
            else:
                # print("No Mixup")
