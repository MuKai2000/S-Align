    ...
    def forward(self, model, sample, per_task, reduce=True, merge_task=False, st_update=False, update_num=None):
        ...
        if per_task == 'st' or (per_task == 'at' and sample['st'] is not None):            # sample_st --> acoustic_encoder --> encoder_out_st
            ...
            if self.at_training and (per_task == 'at' or multi_st):
                ...
                if self.at_level == "sentence":
                    ...
                    if self.mixup_rate < 0.0 and encoder_out_st["mixup_rate"][0] is not None and False:
                        id_st = encoder_out_st["mixup_rate"][0]         # note kkq change here 08.09.2023
                    else:
                    ...
                    # print(" > 1. ", _shape, encoder_output_st.dtype)
                    length = _shape[1]
                    ...
                    if update_num < 100 and update_num % 20 == 0:
                        print(" > st. ", task_input.shape, task_input.unsqueeze(0).shape)
                        pass
                    prob_st = model.task_net(task_input.unsqueeze(0))
                    id_st = torch.zeros(prob_st.shape[0], 1)
                else:
        ...
        if per_task == 'mt' or (per_task == 'at' and sample['mt'] is not None) or multi_st:            # sample_mt --> textual_encoder --> encoder_out_mt
            ...
            if multi_st:
                ...
                if self.mixup_for_whole_model and self.mixup_rate != 0.0 and False:
                ...
                if update_num < 100 and update_num % 20 == 0 and self.mixup_rate != 0.0:
                    print(self.at_nopad, sample["transcript"]['tokens_add_noise'], sample["transcript"]['lengths_add_noise'], sample["transcript"]['ids'])
                    pass
            else: 
            ...
            if self.at_training and (per_task == 'at' or multi_st):
                ...
                encoder_padding_mask_mt = encoder_out_mt["encoder_padding_mask"][0] if self.at_nopad else torch.zeros(encoder_out_mt["encoder_padding_mask"][0].shape).bool().to(encoder_out_mt["encoder_padding_mask"][0].device)  # [bs, len]

                if self.at_level == "sentence":
                    if not self.mixup_for_whole_model and self.mixup_rate < 0.0:
                        # if update_num < 100 and update_num % 20 == 0 and self.mixup_rate != 0.0:
                        #     print(sample["transcript"]['tokens_add_noise'], sample["transcript"]['lengths_add_noise'], sample["transcript"]['ids'])
                    ...
                    if self.mixup_rate < 0.0 and sample["transcript"]['ids'] is not None and False:
                        id_mt = sample["transcript"]['ids'].unsqueeze(-1)
                    else:
                    ...
                    task_input = task_input.transpose(0, 1).reshape(-1, task_input.shape[-1]) # [bs*num_win, dim=512] 
                    if update_num < 100 and update_num % 20 == 0:
                        print(" > mt. ", task_input.shape, task_input.unsqueeze(0).shape)
                        pass
                    prob_mt = model.task_net(task_input.unsqueeze(0))
                    id_mt = torch.ones(prob_mt.shape[0], 1)         # note kkq change here 08.09.2023
                else:
                    raise NotImplementedError
