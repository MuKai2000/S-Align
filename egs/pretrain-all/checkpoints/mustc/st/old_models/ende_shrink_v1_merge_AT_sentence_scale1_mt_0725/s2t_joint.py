...
class AcousticEncoder(Wav2vecEncoder):
    ...
    def forward(self, src_tokens, src_lengths, tbc=True, mixup_rate=0.0, **kwargs):
        ...
        if self.use_ctc_shrink and ft:
            ...
            padding_mask = torch.zeros(padding_mask.shape).bool().cuda()                  # //need_check//
            ...
            # Mix-up
            if mixup_rate > 0.0:                                                                                # //need_check//
                print(tokens.shape, shrink_mask.shape, x.shape)
                tokens_after_shrink = tokens[shrink_mask]   # [bs, len]
                assert tokens_after_shrink.shape == x.shape[:-1]
