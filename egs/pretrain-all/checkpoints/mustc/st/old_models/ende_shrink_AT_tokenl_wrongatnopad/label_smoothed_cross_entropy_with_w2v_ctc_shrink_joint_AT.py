...
class LabelSmoothedCrossEntropywithW2vCtcShrinkJointAT(
    ...
    def forward(self, model, sample, per_task, reduce=True):
        ...
        # ************/ Add AT per-task /************ #
            ...
            st_prob, st_id, mt_prob, mt_id = None, None, None, None # at input
            # st
            if sample['st'] is not None:
                ...
                if self.at_level == "sentence":
                    ...
                    # 去除mask部分
                    encoder_output = st_encoder_out["encoder_out"][0].reshape(-1, st_encoder_out["encoder_out"][0].shape[-1])   # [bs * len, dim=512]
                    encoder_padding_mask = st_encoder_out["encoder_padding_mask"][0].reshape(-1, 1) # [bs * len, 1]
                    encoder_output = encoder_output[torch.any(encoder_padding_mask != True, dim=1)].unsqueeze(0)    # [1, unzero_len, dim=512]
                    # task net 输出
                    st_prob = model.task_net(encoder_output)    # [unzero_len, 1]
                    st_id = torch.zeros(st_prob.shape[0], 1)    # [unzero_len, 1]
            ...
            if sample['mt'] is not None:
                ...
                if self.at_level == "sentence":
                    ...
                    # 去除mask部分
                    encoder_output = mt_encoder_out["encoder_out"][0].reshape(-1, mt_encoder_out["encoder_out"][0].shape[-1])   # [bs * len, dim=512]
                    encoder_padding_mask = mt_encoder_out["encoder_padding_mask"][0].reshape(-1, 1) # [bs * len, 1]
                    encoder_output = encoder_output[torch.any(encoder_padding_mask != True, dim=1)].unsqueeze(0)    # [1, unzero_len, dim=512]
                    # task net 输出
                    mt_prob = model.task_net(encoder_output)    # [unzero_len, 1]
